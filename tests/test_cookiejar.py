...
class TestCookieJarSafe(TestCookieJarBase):
    ...
    def test_duplicate_cookie_different_folder(self) -> None:
        async def make_jar():
            return CookieJar(unsafe=True)
        jar = self.loop.run_until_complete(make_jar())
        jar.update_cookies(SimpleCookie('dup="one"; Domain=duptest.com; Path=/one/;'))
        jar.update_cookies(SimpleCookie('dup="two"; Domain=duptest.com; Path=/two/;'))

        assert len(jar) == 2
        jar.update_cookies(SimpleCookie('dup=""; Domain=duptest.com; Expires=Thu, 01 Jan 1970 00:00:00 GMT'))
        assert len(jar) == 2
        jar.update_cookies(SimpleCookie('dup=""; Domain=duptest.com; Path=/one/; Expires=Thu, 01 Jan 1970 00:00:00 GMT'))
        assert len(jar) == 1
